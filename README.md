...
**ملف واحد مستقل:** يوجد أيضًا ملف **`cheque-extractor.standalone.html`** يحتوي على
التطبيق بالكامل (الواجهة + المكتبات + نموذج التعرّف) في ملف HTML واحد. انسخه إلى أي مكان
وافتحه بالنقر المزدوج — لا حاجة لأي ملفات أخرى. (يُعاد توليده عبر `node tools/build-single-file.js`.)
